...
int
seqfile_iter_parallel_single (seqfile_t *file, seqfile_iter_func_single_t func,
        void *data, seqfile_iter_flags flags)
{
    int global_res = 0;

    #pragma omp parallel shared(file, func, data, flags, global_res) \
        default(none) num_threads(flags.num_threads)
    {
        ...
        while(1) {
            #pragma omp critical
            {
                read_len = seqfile_read(file, seq);
            }
            if (read_len < 0) {
                break;
            }
...
int
seqfile_iter_serial_single (seqfile_t *file, seqfile_iter_func_single_t func,
        void *data, seqfile_iter_flags flags)
{
    int global_res = 0;
    seq_t *seq = seq_create();
...
int
seqfile_iter_parallel_paired (seqfile_t *file1, seqfile_t *file2,
        seqfile_iter_func_paired_t func, void *data, seqfile_iter_flags flags)
{
    int global_res = 0;

    #pragma omp parallel shared(file1, file2, func, data, flags, global_res) \
        default(none) num_threads(flags.num_threads)
    {
        ...
        while(1) {
            #pragma omp critical
            {
                read_len1 = seqfile_read(file1, seq1);
                read_len2 = seqfile_read(file2, seq2);
            }
            if (read_len1 < 0 || read_len2 < 0) {
                break;
            }
            res = (*func)(seq1, seq2, data);
            global_res |= res;
            if (flags.die_quickly) {
                if (!res) {
                    break;
...
int
seqfile_iter_serial_paired (seqfile_t *file1, seqfile_t *file2,
        seqfile_iter_func_paired_t func, void *data, seqfile_iter_flags flags)
{
    int global_res = 0;
    seq_t *seq1 = seq_create();
