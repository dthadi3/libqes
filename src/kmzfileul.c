        ...
        errhandler_t onerr, const char *src, const int line)
{
    ...
    char *end = NULL;
    size_t size = 0;
    if (bufref == NULL || !zfile_readable(file) || sizeref == NULL) {
    ...
    while ((end = memchr(file->bufiter, delim, file->bufflen)) == NULL) {
        tocpy = file->bufflen;
        len += tocpy;
        while (len + 1 >= size) {
        ...
        nextbuf += tocpy;
        nextbuf[0] = '\0'; /* Null-terminate buf */
        if (!__zfile_fill_buffer(file)) {
            return -2;
        }
    }
